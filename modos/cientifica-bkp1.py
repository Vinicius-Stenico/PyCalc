...
class CalculadoraCientifica(tk.Frame):
    ...
    def _inicializar_estado(self) -> None:
        ...
        self.camada_bloqueadora: tk.Frame | None = None
    ...
    def _alternar_menu_trigonometria(self) -> None:
        ...
        self._fechar_menu_funcoes()
        self._criar_camada_bloqueadora()

        self.frame_menu_trigonometria = (
            self._criar_painel_trigonometria()
        )

        self.frame_menu_trigonometria.lift()
    ...
    def _criar_painel_trigonometria(self) -> tk.Frame:
        ...
        painel.place(relx=0.0, rely=0.31, relwidth=0.78)
        ...
        botoes_trigonometria = (
        ("2ⁿᵈ", "sin", "cos", "tan"),
        ("hyp", "sec", "csc", "cot"),
        )

        for linha, valores in enumerate(botoes_trigonometria):
            for coluna, texto in enumerate(valores):
                botao = tk.Button(
                    painel,
                    text=texto,
    ...
    def _selecionar_funcao_trigonometrica(self, valor: str) -> None:
            if valor == "2ⁿᵈ":
            ...
            print("Botão trigonométrico:", valor)
    ...
    def _alternar_menu_funcoes(self) -> None:
        ...
        self.frame_menu_funcoes = self._criar_frame_funcoes()
        self.frame_menu_funcoes.lift()
    ...
    def _fechar_menus_funcoes(self) -> None:
        ...
        self._fechar_menu_funcoes()
        self._remover_camada_bloqueadora()
    ...
    def _tratar_operacao_unaria(self, valor: str, texto_atual: str, expressao: str) -> None:
        if valor == "x²":
            if self.funcoes_secundarias:
                self._calcular_cubo(texto_atual)
            else:
                self._calcular_quadrado(texto_atual)
        elif valor == "|x|":
            self._calcular_valor_absoluto(texto_atual)
        elif valor == "n!":
            self._calcular_fatorial(texto_atual)
        elif valor == "log":
            if self.funcoes_secundarias:
                self._preparar_logaritmo_base_y(texto_atual, expressao)
            else:
                self._calcular_logaritmo(texto_atual)
        elif valor == "√x":
            if self.funcoes_secundarias:
                self._calcular_raiz_cubica(texto_atual)
            else:
            ...
            if self.funcoes_secundarias:
                self._calcular_dois_elevado_x(texto_atual)
            else:
                self._calcular_potencia_base_dez(texto_atual)
        elif valor == "ln":
            if self.funcoes_secundarias:
                self._calcular_exponencial_e(texto_atual)
            else:
                self._calcular_logaritmo_natural(texto_atual)
    ...
    def _criar_camada_bloqueadora(self) -> None:
        if(
            self.camada_bloqueadora is not None
            and self.camada_bloqueadora.winfo_exists()
        ):
            return
        
        self.camada_bloqueadora = tk.Frame(
            self,
            bg=Tema.COR_FUNDO,
            cursor="arrow",
        )

        self.camada_bloqueadora.place(
            relx=0,
            rely=0,
            relwidth=1,
            relheight=1,
        ...
        self.camada_bloqueadora.bind(
            "<Button-1>",
            self._clicar_fora_dos_paineis,
        )

        self.camada_bloqueadora.lift()
        self.frame_funcoes.lift()

    def _remover_camada_bloqueadora(self) -> None:
        if (
            self.camada_bloqueadora is not None
            and self.camada_bloqueadora.winfo_exists()
        ):
            self.camada_bloqueadora.destroy()

        self.camada_bloqueadora = None

    def _clicar_fora_dos_paineis(self, event=None) -> str:
        self._fechar_menu_funcoes()
        return "break"
