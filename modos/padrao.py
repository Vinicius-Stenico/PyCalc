...
class CalculadoraPadrao(InteracaoTecladoVisorMixin, tk.Frame):
    """
    ...
    def _calcular_raiz(self, texto_atual: str) -> None:
        try:
            ...
            expressao_historico = f"√{self.formatar_numero(numero)}"
            self.texto_expressao.set(expressao_historico)
            self.historico.adicionar_expressao(
                expressao=expressao_historico,
                resultado=resultado,
    ...
    def _calcular_fracao(self, texto_atual: str) -> None:
        try:
            ...
            expressao_historico = f"1/{self.formatar_numero(numero)}"
            self.texto_expressao.set(expressao_historico)
            self.historico.adicionar_expressao(
                expressao=expressao_historico,
                resultado=resultado,
